...
const LayerCarousel = ({ slides, slideWidth, duration, autoSlide, cardAlign, bullet, controls }) => {
  ...
  }, [autoSlide, slides.length, distance])
  return (
    slides.length !== 0 && (
      <Swipeable  {...handlers} className="padded-carousel padding-horizontal-xlg slider position-relative overflow-h margin-bottom-sm cursor-pointer">
        <div className="d-flex nowrap align-items-center overflow-h position-relative" style={{width: '100%'}}>
          {slides.map((Slide, index) => {
            if(index + 1 === slides.length) {
              return (
                <div key={index} ref={lastElement} className="slide layer-carousel" style={{minWidth: !cardAlign ? '100%' : 'auto', transform: `translateX(${distance}%)`}}>
                  {Slide}
                </div>
              )
            } else {
              return (
                <div key={index} className="slide layer-carousel" style={{minWidth: !cardAlign ? '100%' : 'auto', transform: `translateX(${distance}%)`}}>
                  {Slide}
                </div>
              )
            }
